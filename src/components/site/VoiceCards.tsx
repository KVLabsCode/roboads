    ...
    <div className="flex min-w-0 flex-col gap-3.5">
      {VOICES.map((v) => (
        <div key={v.id} className="flex items-center gap-[18px] rounded-2xl border-2 border-[#141414] bg-white px-[22px] py-[18px]">
          <button
            onClick={() => play(v.id)}
            aria-label={playing === v.id ? `Pause ${v.name}` : `Play ${v.name}`}
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-1">
            <div className="flex flex-wrap items-baseline gap-x-2.5">
              ...
              <span className="text-[11px] font-bold tracking-[1.5px] text-[#8a8578]">{v.tag}</span>
            </div>
      ...
      <div className="text-[12.5px] text-[#8a8578]">Sample clips. Your robot gets custom lines written with you.</div>
    </div>
  )
}
