        ...
        <div key={v.id} className="flex items-center gap-[18px] rounded-2xl border-2 border-[#141414] bg-white px-[22px] py-[18px]">
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-1">
            <div className="flex flex-wrap items-baseline gap-x-2.5">
              ...
              <span className="text-[12px] font-bold tracking-[1.5px] text-[#141414]">{v.tag}</span>
            </div>
            <div className="text-[15px] italic text-[#141414]">{v.line}</div>
          </div>
          {playing === v.id && <PlayingEq />}
        </div>
      ))}
      <div className="text-[12.5px] text-[#141414]">Sample clips. Your robot gets custom lines written with you.</div>
    </div>
  )
}
