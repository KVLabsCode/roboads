...
export default function ContactSwitcher({ initial }: { initial: 'brand' | 'fleet' | 'agency' }) {
  const [who, setWho] = useState<'brand' | 'fleet' | 'agency'>(initial)
  ...
  return (
    <div className="flex flex-col gap-5">
      <div role="tablist" aria-label="Who are you contacting us as?" className="flex flex-wrap gap-3">
        ...
        <button
          role="tab"
          aria-selected={who === 'agency'}
          onClick={() => setWho('agency')}
          className={`${pill} ${who === 'agency' ? activeCls : idleCls}`}
        >
          I&apos;m an OOH agency
        </button>
      </div>
      ...
      <p className="m-0 text-[15px] leading-normal text-[#141414]">
        {who === 'brand'
          ...
          : who === 'agency'
            ? 'Tell us about your client or campaign and we come back with availability, specs, verified measurement reporting, and partner terms.'
      ...
      {who === 'brand' ? (
        ...
        <LeadForm kind="agency" source="contact" />
      ) : (
